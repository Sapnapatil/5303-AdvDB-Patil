#names on it and information on how to access your api.
#Name :-Sapna Patil,Tejaswi Prakhya
#IP ADDRESS to access API :- http://104.131.64.127:5000/
...
def find_zip(args):
    ...
    result = businessdb.find({"$or": [{"full_address":{"$regex" : '.*' + '89117' + '.*'}},{"full_address":{"$regex" : '.*' + '89122' + '.*'}}]},{"name":1, "full_address":1,"_id":0}).skip(start).limit(end)
...
if __name__ == "__main__":
    #app.run(debug=True,host='67.205.143.154',port=2100)
